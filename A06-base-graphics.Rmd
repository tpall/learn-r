...
## Scatterplot
Scatterplots can be created using `plot()`. If we use `plot()` command with only one variable, we get graph with values versus index. 
```{r}
plot(iris$Sepal.Length)
```

More sensible use of scatterplot is visualizing relationship between variables. Here, we explore the relationship between sepal length and width in different iris species.

```{r}
plot(iris$Sepal.Length, iris$Sepal.Width)
```
It's nice! But we don't know witch dot belongs to which of the three iris species (`r levels(iris$Species)`).
