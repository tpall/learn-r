...
As functional language and [statistical environment, R](https://www.r-project.org/about.html) has pros and cons.    
...
load("data/Available_CRAN_Packages_By_Date_of_Publication_2016-08-17.RData")
t <- t[[1]] # t is list of length 1 containing data.frame
library(dplyr)
tn <- t %>% group_by(Date) %>% summarise(Packages = n())
cap <- paste("The number of available CRAN packages grows exponentially, featuring", length(unique(t$Package)), "available packages as of", t$Date[1],".")
```
...
tn %>% ggplot(aes(ymd(Date), cumsum(Packages), group = 1)) + 
  ...
  ggtitle("Available CRAN Packages By Date of Publication")
```
