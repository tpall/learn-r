...
```{r}
filter(tm_2013, grepl(", Jaan", Nimi)) %>% tbl_df # Nimi is name and is given as 'lastname, firstname'.
```
Ups, we got 'Jaanus'-es too!

Let's add end of a line anchor `$` to get only 'Jaan's and lets use `str_detect` verb:
```{r}
filter(tm_2013, str_detect(Nimi, ", Jaan$")) %>% tbl_df # Nimi is name and is given as 'lastname, firstname'. We use tbl_df to print only head of the table.
```
Good!
