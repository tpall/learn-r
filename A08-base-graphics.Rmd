...
**Scatterplots are also ideal for visualising relationships between independent and dependent variables**. We use R in-house dataset `CO2` showing carbon dioxide uptake in grass plants. We first summarise the dataset by calculating mean uptake per conc and ignore the independent variables Plant, Type, and Treatment:
...
Here we plot plant CO~2~ uptake versus its concentration and add calculated linear model fit to the scatterplot:
```{r}
plot(uptake ~ conc, 
     data = CO2, 
...
R allows combine multiple subplots into one overall graph, using either the `par()` or `layout()` function. 
...
By telling graphics device to create four slots, arranged 2x2, in our plot window, using `par` function argument `mfrow=`, we can tidy up all this information little bit:
```{r}
