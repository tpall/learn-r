...
Hadleyverse's verb to perform this task is `bind_rows`: 
```{r}
library(dplyr)
bind_rows(df1, df2) 
```
...
cbind(1:7, matrix(1:12, nrow = 3)) # vector is subset -> warning
library(dplyr)
bind_cols(data_frame(1:4),data_frame(letters[1:4]))

# dplyr verbs, use for syntax consistency. Much more strict about arguments.
bind_rows()
bind_cols(data_frame(1:7, letters[1:7]), data_frame(sample(c(T,F),7, replace =T)))
