...
When you supply a column name with the `.id=` argument, a new column is created to link each row to its original data frame:
```{r}
bind_rows(df1 = df1, df2 = df2, .id = "id")
```

**Columns don't need to match** when row-binding using `bind_rows`:
```{r}
bind_rows(df1 = df1, df2 = mtcars[11:13, c(1,4,6)], .id = "id")
```
