...
You will not be able to use a package until you load it with `library()` function. 
...
This `library(tidyr)` command makes available all funtions in tidyr package in your R environment.

> R packages are collections of one or more functions with clearly specifed task. 

For example tidyr package contains following `r library(tidyr); length(ls("package:tidyr"))` functions:
...
# or more preferably, as we usually want only to install missing package and to avoid side effects i.e. loading entire package to environment, we use: 
...
help_console(confusionMatrix, "text", lines = 1:25)
```
...
For example `help()` works, but `help` doesn't. 
...
`setwd("C:\mydata")` generates an error. 
Use `setwd("C:/mydata")` or `setwd("C:\\mydata")` instead.
