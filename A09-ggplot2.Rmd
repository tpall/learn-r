...
```{r}
p1 + geom_histogram(position = "dodge", binwidth = 0.3)
```
...
```{r}
p4 + stat_summary(fun.y = mean, geom = "point", shape = 95, color = "red", size = 15) + # mean
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.25) # standard deviation
```

Now we create line graphs with errorbars. First we calculate summary statistics:
```{r}
mpg
mpg_sum <- mpg %>% 
...
First we create linegraph, we want two distinct colors for year variable, therefore we coerce year to factor:
```{r}
...
pd <- position_dodge(1)
p6 + geom_bar(stat = "identity", position = pd)
```
