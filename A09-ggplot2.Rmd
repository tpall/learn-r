...
```{r}
ggplot(data = data.frame(x = c(0:25))) + 
  geom_point(size = 8, aes(x = x, y = x, shape = x)) +
...
```{r}
tm <- mutate(tm_2013, Aeg = unclass(as.duration(hms(Aeg)))/3600) %>% filter(complete.cases(.))
ggplot(tm, aes(Aeg)) + geom_histogram(binwidth = 1/12) # we want binwidth ~5 minutes (1/12 hour)
```
