...
ggplot(mpg, aes(cty, hwy)) +
  geom_point() +
  xlab("City driving (miles per gallon)") + # to set x axis label
...
```{r}
p4_5 <- ggplot(mpg, aes(cty, hwy)) + geom_point() 
p4_5 + expand_limits(x = 0, y = 0) # upper and lower axis limits can be specified by supplying vector of two values
```
...
```{r, warning=TRUE}
p4_5 + scale_y_continuous(limits = c(0, 20))
```
Note the warning!

More simply, we can use `xlim()` and `ylim()`:
```{r}
p4_5 + xlim(15, 20) + ylim(0, 30)
```
...
## Barplots
Barplots are created using `geom_bar` and default stat `counts` needs to changed to `identity`:
```{r}
...
When adding additional variables to a barplot e.g. via `fill =` you need also unstack the bars by specifying `position = position_dodge()`:
```{r}
