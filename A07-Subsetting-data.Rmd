...
+ Index is address used to select elements of vector/matrix/list or data.frame
+ Indexing begins at 1 (not 0) in R
+ Indexing operators in R are square brackets -- '[', `[[` and dollar sign `$`  
+ Important distinction between these operators is that `[` allows select more than one element whereas `[[` and `$` select only one element.
+ Empty index [,] means "all" eg. `a[,1]` means: select all rows and 1st column from `a`.
