...
TAI offers different download formats, but mostly in difficult and useless ragged form (even .csv and .txt files).
However, only "Tabeldieraldusega pealkirjata tekst (.csv)" and "Semikooloneraldusega pealkirjata tekst (.csv)" are in a suitable rectangular format, but lacking column headers.
More often than not, raw data needs some munging. Also here, we have to identify and add column headers separately and fix also character encoding.
...
### `readr` package
You can import tabular data using read_ functions from `readr` package. Compared to base R functions like `read.csv()`, `readr` is much faster and gives more convenient output: it never converts strings to factors, can parse date/times, and it doesn't munge the column names.
...
read_csv2() # reads semi-colon delimited files, 
...
`XLConnect` is a very powerful package to work with .xls(x) files, depends on Java, but has memory limitations: on average PC you never know when your script crashes when importing (even not so) large tables.
